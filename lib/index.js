...
function getImages(posts){
  ...
  var rec=/!\[\]\(\/image\/(.*)\)/;
  var images=[];
  var headers=['default.jpg','small-default.jpg'];
  for(var post in posts){
    if(posts[post].header){
      headers.push(posts[post].header);
      headers.push('small-'+posts[post].header);
    }
    var imgs=posts[post].content.match(re);
    if(imgs){
      var t=imgs.map(function(i){return i.match(rec)[1];});
      images=images.concat(t);
    }
  }
  return {headers:headers,images:images};
}
function rmImages(dir,img,callback){
  fs.readdir(dir,function(err,files){
    var p=files.length;
    if(p==0)return callback();
    function comp(){if(--p==0)callback();}
    files.forEach(function(file){
      if(img.indexOf(file)==-1){
        fs.unlink(dir+file,comp);
      }
      else comp();
    });
  });
...
function server(){
  ...
  app.get('/cleanimage',function(req,res){
    if(cleanning)return;
    cleanning=true;
    var imgs=getImages(posts);
    rmImages(config.image_dir,imgs.images,function(){
      rmImages(config.header_dir,imgs.headers,function(){
        cleanning=false;
        res.send('finish cleanning images');
      });
    });
  });
